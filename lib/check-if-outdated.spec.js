...
describe('Helper: checkNodeVersion', () => {
  ...
  it('does not warn or exit with node v6.3.2', () => {
    mockProcessProperty('version', 'v6.3.2')
    const stdout = catchLog(checkNodeVersion)
    expect(stdout).toBe('')
...
describe('Helper: checkMigrationHelperVersion', () => {
  ...
  it('warns and exits with an old version of antd-migration-helper', done => {
    ...
    catchLog(mod.checkMigrationHelperVersion, stdout => {
      expect(stdout).toMatch('newer version of antd-migration-helper is available')
  ...
  it('does not warn or exit with the latest version of antd-migration-helper', done => {
    const rewire = require('rewire')
    const mod = rewire('./check-if-outdated')
    mod.__set__({
      request: (options, callback) => {
